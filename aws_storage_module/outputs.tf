...
output "ownership_controls" {
  ...
  value = var.ownership_controls ? aws_s3_bucket_ownership_controls.ownership_controls[0] : "Ownership controls not configured"
}

output "bucket_acl" {
  description = "S3 bucket ACL if configured"
  # value       = try(aws_s3_bucket_acl.acl[0], "S3 bucket ACL not available")
  value = var.acl ? aws_s3_bucket_acl.acl[0].acl : "ACL not configured"
}
...
output "bucket_server_side_encryption" {
  ...
  value = var.server_side_encryption ? aws_s3_bucket_server_side_encryption_configuration.server_side_encryption[0].rule : "Server Side Encryption not configured"
}
...
output "bucket_versioning" {
  ...
  value = var.versioning ? aws_s3_bucket_versioning.versioning[0].versioning_configuration : "Versioning not configured"
}
...
output "bucket_lifecycle_rule" {
  ...
  value = var.lifecycle_rule ? (var.versioning ? aws_s3_bucket_lifecycle_configuration.versioning_bucket_config[0].rule : aws_s3_bucket_lifecycle_configuration.bucket_config[0].rule) : "Lifecycle rule not configured" 
}
